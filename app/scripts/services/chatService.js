...
angular.module('ChatFrontendApp')
  .service('Chatservice', function Chatservice(Chatsocket, Userservice, $rootScope, ChatsocketReceiveEvents) {
    var users = {};
    ...
    function addMessage(user, message, when, isMine) {
      user.messages.unshift({
        message: message,
        when: (when ? moment(when) : moment() ).format('dddd, MMMM Do YYYY, h:mm:ss a'),
        isMine: isMine
      });
    }
